...
Future<Uint8List> createPdfFile(
  ...
  Project _project = project.copyWith();
  ...
  if (compressValue != null) {
    // if (compressValue != 1.0) {
      List<File> compressImages = await compressImageFile1(
          List.from(_project.listMedia), compressValue);
  ...
  if (_project.coverPhoto?.frontPhoto != null) {
    ...
    if (compressValue != null ) {
      compressFrontPhoto = (await compressImageFile1(
    ...
    if (_project.paper?.title == "None") {
      var imageFile = File(compressFrontPhoto.path);
  ...
  if (_project.coverPhoto?.backPhoto != null) {
    ...
    if (compressValue != null) {
      compressBackPhoto = (await compressImageFile1(
    ...
    if (_project.paper?.title == "None") {
      var imageFile = File(compressBackPhoto.path);
      var memImage = pw.MemoryImage(imageFile.readAsBytesSync());
      double ratio = memImage.width! / memImage.height!;
      double widthPdf = ((_project.paper?.width) ?? (memImage.width) ?? 0) *
          (valueUnit ?? 0.0);
      ...
      children: layoutExtractList!.map((e) {
        ...
        return pw.Positioned(
            ...
            child: pw.SizedBox(
              ...
              alignment: convertAlignmentToPdfAlignment(
                  project.alignmentAttribute?.alignmentMode),
              // color: convertColorToPdfColor(colorRed),
    ...
    for (int indexColumn = 0;
      ...
      columnWidgets.add(pw.Flexible(
        child: pw.Row(
          ...
          // tinh lai width va height cua tung anh
          double newWidth = widthAndHeight[0];
          double newHeight = widthAndHeight[1];
          if (project.resizeAttribute?.title == LIST_RESIZE_MODE[1].title) {
...
pw.Widget _buildImageWidget(Project project, dynamic imageData,
  ...
  if (imageData == null) {
    ...
    return pw.Image(
      pw.MemoryImage(
        File(imageData.path).readAsBytesSync(),
      ),
      fit: fit,
      width: _width,
      height: _height,
